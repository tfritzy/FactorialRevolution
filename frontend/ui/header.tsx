...
export function Header({ game }: { game: Game }) {
  ...
  return (
    <div className="fixed top-0 left-0 w-full flex flex-row space-x-4 px-4 bg-gray-100 border-b border-blue py-1">
      <div className="text-gold">{state.gold} gold</div>
      <div className="text-red-600">{state.health} health</div>
      <div className="text-black">{state.paused ? "paused" : ""}</div>
